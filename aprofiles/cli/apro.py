...
def run(
    ...
    instruments_types: List[InstrumentType] = typer.Option(
        ["CHM15k", "Mini-MPL"],
        "--instruments-type",
        help="📗 List of specific instruments to be processed.",
    ),
    ...
    apriori_cfg: Path = typer.Option(
        "config", help="📂 Base path for a priori config file."
    ),
    ...
    for instrument_type in CFG["parameters"]:
        ...
        if "cfg" in apriori:
            CFG["parameters"][instrument_type]["inversion"]["apriori"]["cfg"] = str(
                Path(apriori_cfg, apriori["cfg"])
            )
    ...
    for date in dates:
        ...
        if update_data:
            if multiprocessing:
                ...
                for file in track(
                    onlyfiles,
                    description=f"{date.strftime('%Y-%m-%d')}   ",
        ...
        if update_calendar:
            ...
            # add to calendar
            for file in track(
                onlyfiles, description="calendar     ", disable=disable_progress_bar
            ):
        ...
        if update_map:
            ...
            # add to map
            for file in track(
                onlyfiles, description="map          ", disable=disable_progress_bar
            ):
    ...
    if update_climatology:
        ...
        # append IDs
        extra_ids = ['20000-0-10577-0','20008-0-BRN-A','250-1001-75105006-A','20000-0-01001-A','756-2-ERISWIL-A','20000-0-06260-A','20000-0-10868-0','20000-0-07110-A','826-300-1-A','20000-0-00203-A','20000-0-00202-A','20008-0-MCR-A','20000-0-12935-A','20000-0-78990-A','20000-0-07014-A','20000-0-07606-A','20000-0-07145-A','20000-0-06275-A','20000-0-07617-A','20000-0-06267-A','20000-0-03501-A','724-0-0000000000060004-C']
        for el in extra_ids:
...
def l2b(
    ...
    if path_in == Path("data/v-profiles"):
        ...
        path_in = Path(
            path_in, today.strftime("%Y"), today.strftime("%m"), today.strftime("%d")
        )
