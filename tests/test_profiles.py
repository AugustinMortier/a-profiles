...
class TestProfilesData:
    ...
    def test_gaussian_filter(self, subaltitude_profiles):
        filter = subaltitude_profiles.gaussian_filter(
            sigma=0.25, var="attenuated_backscatter_0"
        )
        # test attributes
    ...
    def test_time_avg(self, subaltitude_profiles):
        ...
        # test attributes
        assert (
            avg_profiles.data.attenuated_backscatter_0.attrs["time averaged (minutes)"]
            == 10
        )

    def test_extrapolate_below(self, subaltitude_profiles):
        extrap_profiles = subaltitude_profiles.extrapolate_below(
            var="attenuated_backscatter_0", z=150, method="cst"
        )
        # test attributes
        assert (
            extrap_profiles.data.attenuated_backscatter_0.attrs[
        ...
        assert (
            extrap_profiles.data.attenuated_backscatter_0.attrs[
                "extrapolation_low_layers_method"
            ]
            == "cst"
        )
        # test if single value for 5 first indexes
        assert (
            len(np.unique(extrap_profiles.data.attenuated_backscatter_0.data[0][0:5]))
            == 1
        )

    def test_range_correction(self, subaltitude_profiles):
        range_correc_profiles = subaltitude_profiles.range_correction(
            var="attenuated_backscatter_0"
        )
        # test attributes
        assert (
            range_correc_profiles.data.attenuated_backscatter_0.attrs[
                "range correction"
            ]
            is True
        )
        assert (
            range_correc_profiles.data.attenuated_backscatter_0.attrs["units"] is None
        )

    def test_desaturate_below(self, subtime_profiles):
        desaturate_profiles = subtime_profiles.desaturate_below(
            var="attenuated_backscatter_0", z=4000.0
        )
        # test attributes
        assert (
    ...
    def test_plot(self, subtime_profiles):
        ...
        subtime_profiles.plot(var="calibration_constant_0", show_fig=False)
